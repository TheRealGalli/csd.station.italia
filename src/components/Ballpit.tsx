...
class x {
  ...
  #u(e: any) { this.#s = e[0].isIntersecting; this.#s ? this.#w() : this.#z(); }
  ...
  #A(e: number) {
    const t = Math.tan(o.degToRad((this.cameraFov as number) / 2)) / (this.camera.aspect / e);
  ...
  set postprocessing(e: any) { this.#t = e; this.render = e.render.bind(e); }
  #w() {
    if (this.#n) return;
    const animate = () => {
...
function M(e: PointerEvent) { A.x = e.clientX; A.y = e.clientY; processInteraction(); }
...
function P(e: any, t: DOMRect) { const { position: i, nPosition: s } = e; i.x = A.x - t.left; i.y = A.y - t.top; s.x = (i.x / t.width) * 2 - 1; s.y = (-i.y / t.height) * 2 + 1; }
function D(e: DOMRect) { const { x: t, y: i } = A; const { left: s, top: n, width: o, height: r } = e; return t >= s && t <= s + o && i >= n && i <= n + r; }

const { randFloat: k, randFloatSpread: E } = o;
const F = new a(); const I = new a(); const O = new a(); const V = new a(); const B = new a(); const N = new a(); const _ = new a(); const j = new a(); const H = new a(); const T = new a();

class W {
  config: any; positionData: Float32Array; velocityData: Float32Array; sizeData: Float32Array; center = new a();
  constructor(e: any) { this.config = e; this.positionData = new Float32Array(3 * e.count).fill(0); this.velocityData = new Float32Array(3 * e.count).fill(0); this.sizeData = new Float32Array(e.count).fill(1); this.#R(); this.setSizes(); this.setInitialVelocities(); }
  #R() {
    const { config: e, positionData: t } = this;
    // Distribuzione iniziale ampia e uniforme (no ammasso)
    for (let i = 0; i < e.count; i++) {
      const base = 3 * i;
      const x = (Math.random() * 2 - 1) * (e.maxX * 0.95);
      const y = (Math.random() * 2 - 1) * (e.maxY * 0.95);
      const z = (Math.random() * 2 - 1) * (Math.max(e.maxZ, e.maxSize) * 0.3);
      t[base] = x;
      t[base + 1] = y;
      t[base + 2] = z;
    }
  ...
  setInitialVelocities() {
    const { velocityData: o, config: e } = this;
    for (let i = 0; i < e.count; i++) {
      const base = 3 * i;
      o[base] = k(-0.2, 0.2);
      o[base + 1] = k(-0.2, 0.2);
  ...
  update(e: any) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // leggero "drift" per evitare clustering quando gravity=0
      if (t.gravity === 0) {
        // Gentle flow field for movimento naturale (no jitter)
        const k = 0.15;
        const amp = 0.04 + 0.02 * n[idx]; // leggero scaling con raggio
        B.x += amp * Math.sin((I.y + e.elapsed) * k);
        B.y += amp * Math.cos((I.x + e.elapsed) * k);
      } else {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // Collision con ostacoli UI (AABB in world coords)
      if (Array.isArray(t.obstacles)) {
        for (const obs of t.obstacles as { minX: number; maxX: number; minY: number; maxY: number }[]) {
          const insideX = I.x > (obs.minX + radius) && I.x < (obs.maxX - radius);
          const insideY = I.y > (obs.minY + radius) && I.y < (obs.maxY - radius);
          if (insideX && insideY) {
            // spingi fuori lungo l’asse di penetrazione minore
            const dxLeft = Math.abs(I.x - (obs.minX + radius));
            const dxRight = Math.abs((obs.maxX - radius) - I.x);
            const dyDown = Math.abs(I.y - (obs.minY + radius));
            const dyUp = Math.abs((obs.maxY - radius) - I.y);
            const minPen = Math.min(dxLeft, dxRight, dyDown, dyUp);
            if (minPen === dxLeft) { I.x = obs.minX + radius; B.x = Math.abs(B.x) * t.wallBounce; }
            else if (minPen === dxRight) { I.x = obs.maxX - radius; B.x = -Math.abs(B.x) * t.wallBounce; }
            else if (minPen === dyDown) { I.y = obs.minY + radius; B.y = Math.abs(B.y) * t.wallBounce; }
            else { I.y = obs.maxY - radius; B.y = -Math.abs(B.y) * t.wallBounce; }
            I.toArray(s, base); B.toArray(o, base);
          }
        }
      }
...
class Y extends c {
  uniforms: any; onBeforeCompile2?: (e: any) => void;
  constructor(e: any) {
    super(e);
...
class Z extends d {
  config: any; physics: any; ambientLight: any; light: any;
  ...
  setColors(e: number[]) {
    if (Array.isArray(e) && e.length > 1) {
      const t = (function (e: number[]) {
        let t: number[], i: l[];
        function setColors(e2: number[]) { t = e2; i = []; t.forEach(col => { i.push(new l(col)); }); }
        setColors(e);
        return {
          setColors,
...
function createBallpit(e: HTMLCanvasElement, t: any = {}) {
  ...
  function initialize(e2: any) {
    ...
    const { wWidth, wHeight } = i.size;
    const seeded = { maxX: wWidth / 2, maxY: wHeight, ...e2 };
    s = new Z(i.renderer, seeded);
    i.scene.add(s);
  }
...
type Obstacle = { minX: number; maxX: number; minY: number; maxY: number };

const Ballpit = ({ className = '', followCursor = true, obstacleSelectors = [] as string[], ...props }: any) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const spheresInstanceRef = useRef<any>(null);
  const obstaclesRef = useRef<Obstacle[]>([]);

  // Recompute obstacles in world coords from DOM rects
  const computeObstacles = () => {
    const canvas = canvasRef.current;
    const inst = spheresInstanceRef.current;
    if (!canvas || !inst) return;
    if (!obstacleSelectors || obstacleSelectors.length === 0) {
      obstaclesRef.current = [];
      if (inst.spheres?.config) inst.spheres.config.obstacles = [];
      return;
    }
    const rectCanvas = canvas.getBoundingClientRect();
    const { maxX, maxY } = inst.spheres.config;
    const scaleX = (2 * maxX) / rectCanvas.width;
    const scaleY = (2 * maxY) / rectCanvas.height;
    const list: Obstacle[] = [];
    obstacleSelectors.forEach((sel: string) => {
      document.querySelectorAll<HTMLElement>(sel).forEach(el => {
        const r = el.getBoundingClientRect();
        // Intersection with canvas
        const left = Math.max(0, r.left - rectCanvas.left);
        const right = Math.min(rectCanvas.width, r.right - rectCanvas.left);
        const top = Math.max(0, r.top - rectCanvas.top);
        const bottom = Math.min(rectCanvas.height, r.bottom - rectCanvas.top);
        if (right > left && bottom > top) {
          const minX = (left - rectCanvas.width / 2) * scaleX;
          const maxXW = (right - rectCanvas.width / 2) * scaleX;
          const maxYScreen = rectCanvas.height / 2 - top;
          const minYScreen = rectCanvas.height / 2 - bottom;
          const maxYW = maxYScreen * scaleY;
          const minYW = minYScreen * scaleY;
          list.push({ minX, maxX: maxXW, minY: minYW, maxY: maxYW });
        }
      });
    });
    obstaclesRef.current = list;
    if (inst.spheres?.config) {
      inst.spheres.config.obstacles = list;
    }
  };
  useEffect(() => {
    const canvas = canvasRef.current;
